...
class DipoleRetIterRunner(SimulationRunner):
    """Iterative retarded dipole BEM (BEMRetIter, GMRES + ACA H-matrix).

    Uses DipoleRet excitation with BEMRetIter solver for large-mesh dipole
    simulations where memory-efficient iterative solving is needed.
    ...
    def build_excitation(self) -> Any:
        ...
        if dip is None:
            return DipoleRet(pt, medium = medium_idx)

        return DipoleRet(pt, dip, medium = medium_idx)
    ...
    def run(self,
        ...
        print_info('DipoleRetIter: warming up at enei={:.1f} nm'.format(float(enei[0])))
        t_warm = time.time()
