...
class PlaneWaveStatMirrorRunner(SimulationRunner):
    """Quasistatic plane-wave on a mirror-symmetric particle (BEMStatMirror).
    ...
    def build_excitation(self) -> Any:
        from mnpbem.simulation import PlaneWaveStatMirror

        pol = self.cfg['simulation'].get('polarizations',
                [[1, 0, 0], [0, 1, 0]])

        return PlaneWaveStatMirror(np.asarray(pol, dtype = float))

    def build_solver(self) -> Any:
        from mnpbem.bem import BEMStatMirror

        return BEMStatMirror(self.p)
    ...
    def run(self,
        ...
        print_info('PlaneWaveStatMirror: warming up at enei={:.1f} nm'.format(float(enei[0])))
        t_warm = time.time()
