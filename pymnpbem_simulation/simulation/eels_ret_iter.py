...
class EELSRetIterRunner(SimulationRunner):
    """Iterative retarded EELS BEM (BEMRetIter, GMRES + ACA H-matrix).

    Uses EELSRet excitation with BEMRetIter solver for large-mesh EELS
    ...
    def run(self,
        ...
        print_info('EELSRetIter: warming up at enei={:.1f} nm'.format(float(enei[0])))
        t_warm = time.time()
