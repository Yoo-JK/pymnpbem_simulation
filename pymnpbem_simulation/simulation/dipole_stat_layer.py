...
class DipoleStatLayerRunner(SimulationRunner):
    """Quasistatic dipole excitation near a particle + planar substrate.

    Uses BEMStatLayer + DipoleStatLayer.  Looks up the LayerStructure
    attached to the particle by WithSubstrateBuilder via ``_mnpbem_layer``.
    Computes the total and radiative decay rate for each wavelength.
    ...
    def build_layer(self) -> Any:
        ...
        if layer is None:
            raise RuntimeError(
                '[error] DipoleStatLayerRunner: particle has no <_mnpbem_layer>; '
    ...
    def build_excitation(self,
        ...
        from mnpbem.simulation import DipoleStatLayer
        ...
        dip = DipoleStatLayer(pt, layer, dip = mom)
    ...
    def run(self,
        ...
        print_info('DipoleStatLayer: warming up at enei={:.1f} nm'.format(float(enei[0])))
        t_warm = time.time()
